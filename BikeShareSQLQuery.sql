...
update dbo.bike_tripAll
set RideLengthInSecond = datediff(second,started_at,ended_at) 
where ended_at > started_at

--Convert day_of_week in number to characteristics
...
select member_casual,day_of_week2, day_of_week, count (*) as #ofRide,sum(RideLengthInSecond) As TotalRideLengthInSecond, avg(RideLengthInSecond) As meanOfRideLength, min(RideLengthInSecond) As minOfRideLength, max(RideLengthInSecond) As maxOfRideLength
from dbo.bike_tripAll
where RideLengthInSecond > 0
...
--Copy this data and paste into Excel for Data visualization
select member_casual, month_of_ride,sum(RideLengthInSecond) As TotalRideLengthInSecond,count (*) as #ofRide, avg(RideLengthInSecond) As meanOfRideLength
from dbo.bike_tripAll
